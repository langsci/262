...
\title{Deconstructing exuberant exponence in Batsbi} \abstract{In
  this article, I shall discuss ``exuberant exponence'' in Batsbi
...
\begin{exe}
  ...
  \begin{xlist}
    \ex {\gll don-e-v taylz-i
      \textbf{d}-ek’-\textbf{d}-iy-ẽ \\
    ...
    \ex{ \gll kuyrc’l-e-x qečqečnayrẽ daq’r-i lal-\textbf{d}-iy-ẽ
      makaħǒ\\
      wedding-\textsc{obl-con} various
      food(d/d)-\textsc{pl.abs} go-\textsc{cm-tr-aor} above\\
      ‘At the wedding [they] passed around various foods.’\hfill
      (idem)}
  \end{xlist}
\end{exe}
...
\begin{figure}
    ...
    \foreach \x/\y in {xx/yy, aa/ba, ab/bb, ca/da, cb/db}
      \path[{Stealth[]}-{Stealth[]},dotted, gray, out=-90, in=90] (CRYS1-\x) edge (CRYS1-\y);
    \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \foreach \x/\y in {m2/m21, m3/m31, m4/m41, m5/m51, s1/s11, s2/s21, s3/s31, s4/s41}
      \path[{Stealth[]}-{Stealth[]},dotted, gray, out=-90, in=90] (CRYS2-\x) edge (CRYS2-\y);
    \foreach \x/\y in {p1/p11, p2/p21, p3/p31, p4/p41, p5/p51}
      \draw[{Stealth[]}-{Stealth[]},dotted, gray] (CRYS2-\x) -- (CRYS2-\y);
  \end{tikzpicture}
...
rule introduces both the stem \textit{ek'} (\avm{\tag{b}}) and its
dependent class marker (\avm{\tag{a}}). The same holds for the
transitivity marker \textit{-iy} (\avm{\tag{d}}) and its accompanying
class marker (\avm{\tag{c}}). Each of these complex rules expresses some
property other than class agreement, as indicated by their
\textsc{mud} value, e.g. lexemic identity (\avm{\tag{t}}), or
transitivity (\avm{\tag{u}}), but both are conditioned on the
morphosyntactic property of gender/number agreement (\avm{\tag{w}}),
