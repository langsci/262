...
\begin{table}[htb]
  \centering\smaller
  \tabcolsep.8\tabcolsep %%% BC: This is intentional!
...
\citet{Grice:75} in \refer{ex-grice}.%
\footnote{Grice's example in \refer{ex-grice} violates many of the LSA
...
We indicate the two propositions expressed in \refer{ex-grice} below
the example.  Often, only the proposition in \refer{ex-grice-ai} is
considered what is being ``said,'' or asserted.  The proposition in
\refer{ex-grice-ci} is considered non-asserted.  Under the heading of
...
\gll Je ne dis pas.\\
I NE say not\\
\mytrans{I don't say} \citep[7]{Jespersen:17}
...
\kommentar{ This pattern is also found in what \citet{denbesten:86}
  ...
  \citet{haegeman&zanuttini:91} in \refer{neg-doub}.  The negative
...
\citep{Jespersen:17,denbesten:86,Zeijlstra:diss}. %, see \refer{ex-nc-intro}.
There is also modal concord as in \refer{mod-conc}, where we find two
...
This is shown in \refer{w-kopier}. 
...
\begin{sloppypar}
  ...
  predicate, as in the Yiddish example in \refer{yiddish-front} from
...
considered a nominalisation of the verb, see \refer{ex-coc}.  As the
...
sentence but need to be interpreted as a single unit, a \emph{polyadic
  quantifier}. This is illustrated in \refer{ex-different}, from \citet{Keenan:92},
...
These include \emph{gapping}, see \refer{ex-gapping}, and \emph{argument cluster coordination}, as in \refer{ex-acc}, both examples are taken from \citet{Kubota:Levine:16-Gapping}.
